...
В результате выполнения запроса возвращаются начертания выбранной категории, в которых найден заданный символ для того же языка, что и у исходного начертания. Соединения в запросе последовательно связывают начертания с гарнитурами, категориями и символами, а затем сравнивают найденные символы с набором символов исходного начертания. Порядок выполнения первого запроса описан на таблице \ref{table:query-1-execution}.

\begin{table}[H]
    \centering
    \caption{Порядок выполнения первого запроса}
    \label{table:query-1-execution}
    \begin{tblr}{
        colspec = {Q[1.2cm,c]X[l]X[l]},
        row{1} = {font=\bfseries},
    }
        \hline
        Шаг & Операция & Описание \\
        \hline
        1 & \mintinline{text}{Seq Scan (Typeface source_t)} & \\
        2 & \mintinline{text}{Index Only Scan (Symbol source_s)} & \\
        3 & \mintinline{text}{Index Only Scan (Symbol s)} & \\
        4 & \mintinline{text}{Index Scan (Typeface t) } & \\
        5 & \mintinline{text}{Index Scan (Font_family ff)} & \\
        6 & \mintinline{text}{Index Scan (Category c)} & \\
        7 & \mintinline{text}{Sort + Unique} & \\
        \hline
    \end{tblr}
\end{table}
