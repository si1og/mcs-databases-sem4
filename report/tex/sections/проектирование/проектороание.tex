...
Для создания таблиц базы данных был написан SQL-скрипт для СУБД PostgreSQL. В проекте используется база данных \mintinline{text}{typography}, запущенная в контейнере Docker. Контейнер базы данных создаётся на основе образа Postgretext и доступен на локальном порту \mintinline{text}{5432}.
...
Структура связей между таблицами, используемыми при программной реализации базы данных, представлена на уменьшенной схеме базы данных на рисунке~\ref{fig:db-schema-small}.

Для заполнения базы данных тестовыми данными используется программа на TypeScript. Подключение к базе данных выполняется при помощи библиотеки \mintinline{text}{pg}. Для генерации правдоподобных значений используется библиотека \mintinline{text}{@faker-js/faker}. Запуск скриптов выполняется через \mintinline{text}{tsx}, что позволяет запускать TypeScript-файлы без отдельной ручной компиляции.
...
Количество записей в каждой таблице приведено в таблице~\ref{table:record-count}.

\begin{table}[H]
    \centering
    \caption{Количество записей в таблицах}
    \label{table:record-count}
    \begin{tblr}{
        colspec = {X[2,l]X[1,c]},
        hline{1-2},
        hline{Z},
        ...
        \mintinline{text}{Category} & 10 \\
        \mintinline{text}{Format} & 6 \\
        \mintinline{text}{Language} & 50 \\
        \mintinline{text}{Font_family} & 100 \\
        \mintinline{text}{Typeface} & 500 \\
        \mintinline{text}{Alphabet} & 1000 \\
    \end{tblr}
\end{table}

\clearpage
\refstepcounter{figure}
\label{fig:db-schema-small}
\includepdf[
    pages=1,
    landscape,
    fitpaper,
