...
% TODO: Воспроизводит -> поддерживает глифы (алфавит)
...
\node[attribute] (g4) [below right of=garn] {Издательство} edge (garn);
\node[attribute] (g5) [below left of=garn] {Лицензия} edge (garn);
\node[attribute] (g6) [right=3em of g4] {Число начертаний} edge (garn);
...
\node[attribute] (k4) [right of=kat] {Число шрифтов} edge (kat);
% выдаёт
...
\node[attribute] (n4) [left of=nach] {Формат файла} edge (nach);
...
% поддерживает
\node[relationship] (obl) [below=12em of nach] {Воспроизводит}
    edge node[left] {1} (nach);

% ПОДДЕРЖКА ЯЗЫКА
\node[entity] (podder) [below=8em of obl] {Поддержка языка};
...
\node[entity] (isp) [below right=18em and 18em of nach] {Использование};
...
\node[relationship] (r_fix) [below right=8em and 8em of nach] {Регистрируется};
\draw[link] (nach) -- node[above left] {1} (r_fix);
\draw[link] (r_fix) -- node[above right] {N} (isp);

% размещено на
\node[relationship] (r_razm) [right=8em of isp] {Размещено}
...
\node[entity] (os) [below=8em of r_ust] {ОС};
...
\node[text width=20em, anchor=north west, font=\small] (goals) at ([xshift=-15em, yshift=0]current bounding box.north east) {
    ...
    \begin{enumerate}
        ...
        \item Начертание воспроизводит поддержку языка.
        \item Поддержка языка охватывает язык.
        \item Начертание хранится в формате.
        \item Начертание регистрируется в используемых.
        \item Используемое начертание размещается на рабочей станции.
