...
FROM Font_family ff
JOIN Format new_f ON new_f.name = 'OpenType' -- начертание A
JOIN Format old_f ON old_f.name = 'TrueType' -- Гарнитура B
WHERE ff.id_font_family = t.id_font_family
  AND old_f.id_format = t.id_format
  AND t.name = 'Family 1 Thin' -- формат C
  AND ff.name = 'Striking Horde 1' -- формат D
RETURNING
  t.id_typeface,
